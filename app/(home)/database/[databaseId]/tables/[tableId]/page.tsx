...
export default function AddTablePage() {
	...
	const schemaId = Array.isArray(params.schemaId) ? params.schemaId[0] : params.schemaId;
	...
	const addTable = async (values: z.infer<typeof TableWithArrayColumnSchema>) => {
		try {
			// Count primary keys before submission
			...
			if ('data' in table && table.data) {
				router.push(`/app/database/${databaseId}/tables/${table.data.id}`);
			} else {
							...
							<div>
								{fields.map((field, index) => (
									<div key={field.id} className='flex items-center justify-evenly gap-4 mb-4 '>
										...
										<Button variant='outline' size='icon' type='button' onClick={() => remove(index)} className='mt-8'>
